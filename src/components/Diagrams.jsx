...
import { WithSnippet } from './WithSnippet';
...
const DemoNode = ({ children, icon, bgColor, borderColor, textColor }) => (
    <div className={`flex items-center justify-center p-1 sm:p-2 rounded-lg w-full text-wrap`}
        ...
        {icon && <span className="mr-0.5 sm:mr-1" style={typography.diagramIcon}>{icon}</span>}
...
const DemoArrow = ({ label, direction = 'r', arrowColor }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center px-1 sm:px-2 md:px-4 flex-shrink-0">
            <div className={`transform ${rotations[direction]}`} 
                 style={{ 
                     color: arrowColor,
                     ...typography.diagramArrow
                 }}>
                →
            </div>
            {label && (
...
const DemoGroup = ({ children, title, borderColor, textColor, bgColor, flexWrap = 'flex-wrap' }) => (
    <div className={`border-2 border-dashed rounded-lg p-1 sm:p-2 md:p-3 flex flex-col items-center justify-start flex-shrink flex-grow basis-1/5 sm:basis-1/4 min-w-0 max-w-[120px] sm:max-w-[160px] md:max-w-[200px]`}
        ...
        <div className={`flex items-center justify-center space-x-1 sm:space-x-2 space-y-1 sm:space-y-2 w-full ${flexWrap}`}>
            {children}
        </div>
    </div>
);
